...
export default async function EventGuestsPage({
  ...
  if (!detail) notFound();
  const { event, joined, waitlist, ticketTypeName, takenSpots, totalJoined, totalWaitlist, totalPending, checkedInCount } =
    detail;
    ...
    <div className="max-w-md">
      ...
      <div className="mb-8 rounded-md border border-grey/15 p-4">
        ...
        <ul className="flex flex-col gap-2">
          {joined.map((a) => (
            <li key={a.id} className="flex flex-col gap-1.5 rounded-full border border-grey/15 px-3 py-1.5">
              <div className="flex items-center gap-3 text-sm">
                ...
                {ticketTypeName(a.ticketTypeId) && (
                  <span className="rounded-full bg-grey/10 px-2 py-0.5 text-xs text-grey">
                    {ticketTypeName(a.ticketTypeId)}
                  </span>
                )}
                {a.checkedInAt && (
                  <span className="rounded-full bg-deep-orange/10 px-2 py-0.5 text-xs text-deep-orange">
                    Checked in
                  </span>
                )}
              </div>
      ...
      {waitlist.length > 0 && (
        <div className="mb-8 rounded-md border border-grey/15 p-4">
          ...
          <ul className="flex flex-col gap-2">
            {waitlist.map((a) => (
              <li key={a.id} className="flex items-center gap-3 rounded-full border border-grey/15 px-3 py-1.5 text-sm">
                <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-orange/10 font-display text-xs text-ink">
                  {a.member.name.charAt(0)}
                </span>
                <span className="text-ink">
